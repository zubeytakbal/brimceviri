...
} from "../../converter/localizedSpanishConversionPages";
import { findSpanishUnitPage } from "../../converter/localizedSpanishUnitPages";
...
export function generateStaticParams() {
  return spanishConversionPages.map((page) => ({
    slug: page.slug,
  }));
}
...
export async function generateMetadata({
  ...
  const page = findSpanishConversionPage(slug);
  ...
  return {
    ...
    alternates: {
      ...
      ...buildFullLanguageAlternates(`/es/${page.slug}`),
    },
    openGraph: {
      title,
      description,
      url: buildSiteUrl(`/es/${page.slug}`),
...
export default async function SpanishConversionPage({ params }: PageProps) {
  ...
  const reversePage = findSpanishConversionPage(page.reverseSlug);
  const fromUnitInfo = findSpanishUnitPage(page.category, page.fromUnit);
  const toUnitInfo = findSpanishUnitPage(page.category, page.toUnit);
  const sources = getUnitSources(page.category);

  const relatedConversions = spanishConversionPages
    .filter(
      (relatedPage) =>
