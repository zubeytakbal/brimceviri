...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/es-419/unit-guides/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/es-419/unit-guides/${unitPage.slug}`),
    },
    openGraph: {
      title: `${unitPage.name} — ${unitPage.categoryName}`,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/es-419/unit-guides/${unitPage.slug}`),
    ...
    <main className="all-conversions-page" lang="es-419">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Ruta de navegación">
          ...
          {categoryPage && (
            <>
              <Link href={`/es-419/categories/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">&rsaquo;</span>
            </>
