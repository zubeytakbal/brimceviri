...
function findBySlug(slug: string) {
  return danishUnitPages.find((page) => page.slug === slug);
}

export function generateStaticParams() {
  return danishUnitPages.map((page) => ({ slug: page.slug }));
}
...
export async function generateMetadata({
  ...
  if (!unitPage) {
    return {
      title: "Enheden blev ikke fundet",
  ...
  return {
    ...
    alternates: {
      canonical: `/da/enhedsguider/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/da/enhedsguider/${unitPage.slug}`),
    },
    openGraph: {
      title: `${unitPage.name} — ${unitPage.categoryName}`,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/da/enhedsguider/${unitPage.slug}`),
...
export default async function DanishUnitPage({ params }: PageProps) {
  const { slug } = await params;
  const unitPage = findBySlug(slug);
  ...
  const categoryPage = danishCategoryPages.find(
    ...
    <main className="all-conversions-page" lang="da">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Sidenavigation">
          ...
          {categoryPage && (
            <>
              <Link href={`/da/kategorier/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">&rsaquo;</span>
            </>
          ...
          {categoryPage && (
            <section className="conversion-section">
              <p>
                <Link
                  className="text-link"
                  href={`/da/kategorier/${categoryPage.slug}`}
                >
                  Se andre enheder i kategorien {categoryPage.title}
                </Link>
              </p>
            </section>
          )}
