...
import { solveAmpToKwUz } from "../../converter/ampToKwUz";
import type { ElectricalSystemType } from "../../converter/ampToKw";
import { formatUzValue } from "../../converter/pressureForceAreaUz";

type AmpToKwCalculatorUzProps = {
  eyebrow: string;
  title: string;
  description: string;
...
export default function AmpToKwCalculatorUz({
  eyebrow,
  title,
  description,
  resultHeading,
}: AmpToKwCalculatorUzProps) {
  ...
  const result = useMemo(
    ...
    [currentUnit, currentValue, efficiencyValue, powerFactorValue, systemType, voltageUnit, voltageValue]
  );
            ...
            <div className="engineering-inputs">
              <label className="engineering-field">
                ...
                <div className="engineering-field-row">
                  ...
                  >
                    {currentUnitDefinitions.map((unit) => (
                      <option key={unit.symbol} value={unit.symbol}>
                        {currentUnitNamesUz[unit.enName]?.name ?? unit.enName} ({unit.symbol})
                      </option>
                    ))}
                  </select>
                </div>
        ...
        >
          ...
          {!result.error && result.powerValue !== null && (
            <p className="engineering-si-note">
              {copy.automaticResultUnit}: {result.powerUnit}
            </p>
          )}
