...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/no/unit-guides/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/no/unit-guides/${unitPage.slug}`),
    },
    openGraph: {
      title: `${unitPage.name} — ${unitPage.categoryName}`,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/no/unit-guides/${unitPage.slug}`),
    ...
    <main className="all-conversions-page" lang="no">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Sidenavigering">
          ...
          {categoryPage && (
            <>
              <Link href={`/no/categories/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">&rsaquo;</span>
            </>
          ...
          {categoryPage && (
            <section className="conversion-section">
              <p>
                <Link
                  className="text-link"
                  href={`/no/categories/${categoryPage.slug}`}
                >
