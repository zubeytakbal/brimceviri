...
const targetLabels: Record<PpmTarget, string> = {
  ppm: "ppm",
...
export default function PpmCalculator() {
  ...
  const [ppmInput, setPpmInput] = useState("5");
  const [cozunenInput, setCozunenInput] = useState("5");
  const [cozucuInput, setCozucuInput] = useState("1000000");
  ...
  const result = useMemo(
    () =>
      calculatePpm({
        target,
        ppm: parseNumericValue(ppmInput),
        cozunenKutlesi: cozunenGram,
        cozucuKutlesi: cozucuGram,
      }),
    [target, ppmInput, cozunenGram, cozucuGram]
  );
      ...
      return [
        {
          title: "1. Adım — ppm",
          lines: [
            "ppm = (çözünen / çözelti) × 10⁶",
            `ppm = (${formatNumber(result.cozunenKutlesi)} g / ${formatNumber(result.cozeltiKutlesi)} g) × 10⁶`,
            `ppm ≈ ${formatNumber(result.ppm, 2)}`,
          ],
        },
      ];
    }
      ...
      return [
        {
          ...
          lines: [
            "çözünen = (ppm × çözücü) / (10⁶ - ppm)",
            `çözünen = (${formatNumber(result.ppm, 2)} × ${formatNumber(result.cozucuKutlesi)} g) / (10⁶ - ${formatNumber(result.ppm, 2)})`,
    ...
    return [
      {
        ...
        lines: [
          "çözücü = çözünen × (10⁶ - ppm) / ppm",
          `çözücü = ${formatNumber(result.cozunenKutlesi)} g × (10⁶ - ${formatNumber(result.ppm, 2)}) / ${formatNumber(result.ppm, 2)}`,
    ...
    <div className="category-general-converter">
      <div className="engineering-calculator-card">
        <p className="calculator-usage-hint">
          ...
          çözücü), eksik olanı ve ppm değerini biz bulalım. ppm, kütlece
          yüzdenin çok küçük derişimler için 10⁶ ile ölçeklendirilmiş
          hâlidir.
        </p>
        ...
        <div className="engineering-targets">
          ...
          <div className="engineering-target-grid">
            {(Object.keys(targetLabels) as PpmTarget[]).map((key) => (
              <button
                key={key}
                type="button"
          ...
          {target !== "ppm" && (
            <label className="category-general-converter-field">
              ...
              <input
                ...
                type="text"
                value={ppmInput}
                onChange={(event) => setPpmInput(event.target.value)}
              />
            </label>
          )}
      ...
      >
        {!result ? (
          ...
          <div className="paint-calculator-result-grid">
            <div>
              <span>ppm</span>
              <strong>{formatNumber(result.ppm, 2)}</strong>
            </div>
            <div>
              <span>Çözünen Kütlesi</span>
              <strong>
