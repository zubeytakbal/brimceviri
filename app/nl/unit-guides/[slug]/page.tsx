...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/nl/unit-guides/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/nl/unit-guides/${unitPage.slug}`),
    },
    openGraph: {
      title,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/nl/unit-guides/${unitPage.slug}`),
    ...
    <main className="all-conversions-page" lang="nl">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Kruimelpad">
          ...
          {categoryPage && (
            <>
              <Link href={`/nl/categories/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">›</span>
            </>
          ...
          {categoryPage && (
            <section className="conversion-section">
              <p>
                <Link
                  className="text-link"
                  href={`/nl/categories/${categoryPage.slug}`}
                >
