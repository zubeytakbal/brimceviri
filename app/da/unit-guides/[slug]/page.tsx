...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/da/unit-guides/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/da/unit-guides/${unitPage.slug}`),
    },
    openGraph: {
      title: `${unitPage.name} — ${unitPage.categoryName}`,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/da/unit-guides/${unitPage.slug}`),
    ...
    <main className="all-conversions-page" lang="da">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Sidenavigation">
          ...
          {categoryPage && (
            <>
              <Link href={`/da/categories/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">&rsaquo;</span>
            </>
          ...
          {categoryPage && (
            <section className="conversion-section">
              <p>
                <Link
                  className="text-link"
                  href={`/da/categories/${categoryPage.slug}`}
                >
