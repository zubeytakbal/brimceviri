...
export default function HindiCategoriesPage() {
  const categories = [...new Set(hindiConversionPages.map((page) => page.categoryName))];
  return (
    <main className="all-conversions-page" lang="hi">
      <div className="all-conversions-shell">
        ...
        {categories.map((category) => <section className="category-article-content" key={category}>
          <h2>{category}</h2>
          <ul>{hindiConversionPages.filter((page) => page.categoryName === category).map((page) => <li key={page.slug}><Link href={`/hi/${page.slug}`}>{page.fromName} से {page.toName}</Link></li>)}</ul>
        </section>)}
      </div>
    </main>
  );
