...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/sv/enhetsguider/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/sv/enhetsguider/${unitPage.slug}`),
    },
    openGraph: {
      title: `${unitPage.name} — ${unitPage.categoryName}`,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/sv/enhetsguider/${unitPage.slug}`),
    ...
    <main className="all-conversions-page" lang="sv">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Sidnavigering">
          ...
          {categoryPage && (
            <>
              <Link href={`/sv/kategorier/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">&rsaquo;</span>
            </>
          ...
          {categoryPage && (
            <section className="conversion-section">
              <p>
                <Link
                  className="text-link"
                  href={`/sv/kategorier/${categoryPage.slug}`}
                >
