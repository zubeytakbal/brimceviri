...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/nl/categories/${categoryPage.slug}`,
      ...buildFullLanguageAlternates(`/nl/categories/${categoryPage.slug}`),
    },
    openGraph: {
      title,
      description: categoryPage.description,
      url: buildSiteUrl(`/nl/categories/${categoryPage.slug}`),
    ...
    <main className="all-conversions-page" lang="nl">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Kruimelpad">
          ...
          <Link href="/nl/categories">Categorieën</Link>
          <span aria-hidden="true">›</span>
        ...
        {units.length > 0 && (
          <section className="conversion-section">
            ...
            <ul className="related-conversion-list">
              {units.map((unit) => (
                <li key={unit.slug}>
                  <Link href={`/nl/unit-guides/${unit.slug}`}>
