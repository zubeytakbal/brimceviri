...
function findBySlug(slug: string) {
  return es419UnitPages.find((page) => page.slug === slug);
}

export function generateStaticParams() {
  return es419UnitPages.map((page) => ({ slug: page.slug }));
}
...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `/es-419/guias-de-unidades/${unitPage.slug}`,
      ...buildFullLanguageAlternates(`/es-419/guias-de-unidades/${unitPage.slug}`),
    },
    openGraph: {
      title: `${unitPage.name} — ${unitPage.categoryName}`,
      description: unitPage.shortDescription,
      url: buildSiteUrl(`/es-419/guias-de-unidades/${unitPage.slug}`),
...
export default async function Es419UnitPage({ params }: PageProps) {
  const { slug } = await params;
  const unitPage = findBySlug(slug);
    ...
    <main className="all-conversions-page" lang="es-419">
      <div className="all-conversions-shell">
        <nav className="breadcrumbs" aria-label="Ruta de navegación">
          ...
          {categoryPage && (
            <>
              <Link href={`/es-419/categorias/${categoryPage.slug}`}>
                {categoryPage.title}
              </Link>
              <span aria-hidden="true">&rsaquo;</span>
            </>
          ...
          {categoryPage && (
            <section className="conversion-section">
              <p>
                <Link
                  className="text-link"
                  href={`/es-419/categorias/${categoryPage.slug}`}
                >
