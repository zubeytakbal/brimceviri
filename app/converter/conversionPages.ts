import { unitRegistry, type UnitRegistryEntry } from "./unitRegistry";

export type ConversionPage = {
  slug: string;
  category: string;
  fromUnit: string;
  toUnit: string;
  fromName: string;
  toName: string;
  formula: string;
  explanation: string;
  exampleValues: number[];
  reverseSlug: string;
  englishSlug?: string;
  englishReverseSlug?: string;
};

type ConversionPairDefinition = {
  category: string;
  firstId: string;
  secondId: string;
  firstExamples: number[];
  secondExamples: number[];
  englishOverride?: { forward: string; reverse: string };
  // Historical hand-authored factor/name that diverges from what the
  // registry's SI factor or canonical tr name would otherwise produce.
  // Kept only where the live site's existing text depends on it; see the
  // usage sites below for why each one exists.
  factorOverride?: number;
  nameOverrides?: Record<string, string>;
};

function getRegistryUnit(id: string): UnitRegistryEntry {
  const entry = unitRegistry.find((unit) => unit.id === id);

  if (!entry) {
    throw new Error(`unitRegistry: unknown unit id "${id}"`);
  }

  return entry;
}

function formatValue(value: number) {
  // 12 ondalik basamaklik sinir, quad BTU gibi asiri buyuk/kucuk faktorlerde
  // (orn. 1 J = ~9.48e-19 quad BTU) sonucu sessizce "0"a yuvarlar --
  // yanlis bir "0" ifadesi aciklama metnine/SSS semasina sizmasin diye
  // bu araliklarda bilimsel gosterime dusulur.
  if (
    value !== 0 &&
    (Math.abs(value) >= 1_000_000_000 || Math.abs(value) < 0.000001)
  ) {
    return value.toExponential(8);
  }

  return Number(value.toPrecision(12)).toLocaleString("tr-TR", {
    maximumFractionDigits: 12,
  });
}

function createOperation(fromName: string, toName: string, factor: number) {
  if (factor >= 1) {
    return `${toName} = ${fromName} × ${formatValue(factor)}`;
  }

  return `${toName} = ${fromName} ÷ ${formatValue(1 / factor)}`;
}

function createExplanation(
  fromName: string,
  toName: string,
  fromUnit: string,
  toUnit: string,
  factor: number
) {
  const operation =
    factor >= 1
      ? `${formatValue(factor)} ile çarpılır`
      : `${formatValue(1 / factor)} değerine bölünür`;

  return (
    `${fromName} ile ${toName} arasındaki dönüşümde ` +
    `${fromName.toLocaleLowerCase("tr-TR")} değeri ${operation}. ` +
    `1 ${fromUnit}, ${formatValue(factor)} ${toUnit} değerine eşittir.`
  );
}

function createConversionPair(
  definition: ConversionPairDefinition
): ConversionPage[] {
  const { category, firstExamples, secondExamples, englishOverride } =
    definition;
  const first = getRegistryUnit(definition.firstId);
  const second = getRegistryUnit(definition.secondId);

  if (
    first.siFactor === undefined ||
    second.siFactor === undefined ||
    !first.tr ||
    !second.tr
  ) {
    throw new Error(
      `conversionPages: ${definition.firstId}/${definition.secondId} missing siFactor or tr name`
    );
  }

  const firstName = definition.nameOverrides?.[definition.firstId] ?? first.tr.name;
  const secondName = definition.nameOverrides?.[definition.secondId] ?? second.tr.name;

  const secondPerFirst =
    definition.factorOverride ?? first.siFactor / second.siFactor;
  const firstPerSecond = 1 / secondPerFirst;

  const forwardSlug = `${first.tr.slug}-${second.tr.slug}`;
  const reverseSlug = `${second.tr.slug}-${first.tr.slug}`;

  const forwardEnglishSlug =
    englishOverride?.forward ??
    (first.en && second.en
      ? `${first.enConversionSlug ?? first.en.slug}-to-${second.enConversionSlug ?? second.en.slug}`
      : undefined);
  const reverseEnglishSlug =
    englishOverride?.reverse ??
    (first.en && second.en
      ? `${second.enConversionSlug ?? second.en.slug}-to-${first.enConversionSlug ?? first.en.slug}`
      : undefined);

  const forwardPage: ConversionPage = {
    slug: forwardSlug,
    category,
    fromUnit: first.symbol,
    toUnit: second.symbol,
    fromName: firstName,
    toName: secondName,
    formula: createOperation(firstName, secondName, secondPerFirst),
    explanation: createExplanation(
      firstName,
      secondName,
      first.symbol,
      second.symbol,
      secondPerFirst
    ),
    exampleValues: firstExamples,
    reverseSlug,
    englishSlug: forwardEnglishSlug,
    englishReverseSlug: reverseEnglishSlug,
  };

  const reversePage: ConversionPage = {
    slug: reverseSlug,
    category,
    fromUnit: second.symbol,
    toUnit: first.symbol,
    fromName: secondName,
    toName: firstName,
    formula: createOperation(secondName, firstName, firstPerSecond),
    explanation: createExplanation(
      secondName,
      firstName,
      second.symbol,
      first.symbol,
      firstPerSecond
    ),
    exampleValues: secondExamples,
    reverseSlug: forwardSlug,
    englishSlug: reverseEnglishSlug,
    englishReverseSlug: forwardEnglishSlug,
  };

  return [forwardPage, reversePage];
}

const pairDefinitions: ConversionPairDefinition[] = [
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "kilometre",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [1, 2, 5, 10, 25, 50, 100],
  },
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "santimetre",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 100],
    secondExamples: [1, 10, 50, 100, 250, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "milimetre",
    firstExamples: [0.01, 0.1, 0.5, 1, 2, 5, 10],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "desimetre",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 100],
    secondExamples: [1, 5, 10, 20, 50, 100, 1000],
  },
  {
    category: "uzunluk",
    firstId: "desimetre",
    secondId: "santimetre",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 100],
    secondExamples: [1, 5, 10, 20, 50, 100, 1000],
  },
  {
    category: "uzunluk",
    firstId: "desimetre",
    secondId: "milimetre",
    firstExamples: [0.01, 0.1, 0.5, 1, 2, 5, 10],
    secondExamples: [1, 10, 50, 100, 200, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "nanometre",
    secondId: "pikometre",
    firstExamples: [0.001, 0.01, 0.1, 1, 2, 5, 10],
    secondExamples: [1, 10, 100, 500, 1000, 2000, 10000],
  },
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "pikometre",
    firstExamples: [0.0000000001, 0.000000001, 0.00000001],
    secondExamples: [1, 100, 1000, 10000, 100000, 1000000],
  },
  {
    category: "uzunluk",
    firstId: "kilometre",
    secondId: "mil",
    firstExamples: [1, 5, 10, 25, 50, 100, 500],
    secondExamples: [1, 5, 10, 25, 50, 100, 500],
  },
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "fit",
    firstExamples: [1, 2, 5, 10, 25, 50, 100],
    secondExamples: [1, 5, 10, 25, 50, 100, 500],
  },
  {
    category: "uzunluk",
    firstId: "santimetre",
    secondId: "milimetre",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 100],
    secondExamples: [1, 5, 10, 20, 50, 100, 1000],
  },
  {
    category: "uzunluk",
    firstId: "milimetre",
    secondId: "inc",
    firstExamples: [0.66, 1, 5, 10, 25.4, 50],
    secondExamples: [0.026, 0.0394, 0.1969, 0.3937, 1, 1.9685],
  },
  {
    category: "uzunluk",
    firstId: "santimetre",
    secondId: "inc",
    firstExamples: [1, 2.54, 10, 25, 50, 100, 250],
    secondExamples: [1, 2, 4, 10, 12, 24, 36],
  },
  {
    category: "uzunluk",
    firstId: "metre",
    secondId: "yarda",
    firstExamples: [1, 2, 5, 10, 25, 50, 100],
    secondExamples: [1, 5, 10, 25, 50, 100, 250],
  },
  {
    category: "uzunluk",
    firstId: "fit",
    secondId: "inc",
    firstExamples: [1, 2, 3, 5, 10, 25, 50],
    secondExamples: [1, 6, 12, 24, 36, 48, 120],
  },
  {
    category: "uzunluk",
    firstId: "mil",
    secondId: "fit",
    firstExamples: [0.25, 0.5, 1, 2, 5, 10, 26.2],
    secondExamples: [1, 10, 100, 5280, 10000, 25000, 52800],
  },
  {
    category: "uzunluk",
    firstId: "furlong",
    secondId: "metre",
    firstExamples: [1, 2, 4, 6, 8, 10],
    secondExamples: [100, 200, 400, 600, 800, 1000],
  },
  {
    category: "kutle",
    firstId: "kilogram",
    secondId: "gram",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 25],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "kutle",
    firstId: "gram",
    secondId: "miligram",
    firstExamples: [0.001, 0.01, 0.1, 0.5, 1, 5, 10],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "kutle",
    firstId: "kilogram",
    secondId: "pound",
    firstExamples: [1, 2, 5, 10, 25, 50, 100],
    secondExamples: [1, 5, 10, 25, 50, 100, 500],
  },
  {
    category: "kutle",
    firstId: "gram",
    secondId: "grain",
    firstExamples: [0.0648, 0.5, 1, 5, 10, 50],
    secondExamples: [1, 7.72, 15.43, 77.16, 154.32, 771.6],
  },
  {
    category: "kutle",
    firstId: "ton",
    secondId: "kilogram",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 25],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "kutle",
    firstId: "gram",
    secondId: "ons",
    firstExamples: [1, 5, 10, 25, 50, 100, 500],
    secondExamples: [1, 2, 4, 8, 16, 32, 64],
  },
  {
    category: "kutle",
    firstId: "pound",
    secondId: "ons",
    firstExamples: [0.5, 1, 2, 5, 10, 25, 50],
    secondExamples: [1, 2, 4, 8, 16, 32, 64],
  },
  {
    category: "kutle",
    firstId: "okka",
    secondId: "gram",
    firstExamples: [0.25, 0.5, 1, 2, 5, 10, 50],
    secondExamples: [100, 250, 500, 1000, 2500, 5000, 10000],
  },
  {
    category: "kutle",
    firstId: "okka",
    secondId: "kilogram",
    firstExamples: [0.25, 0.5, 1, 2, 5, 10, 50],
    secondExamples: [0.5, 1, 2, 5, 10, 25, 50],
  },
  {
    category: "kutle",
    firstId: "dirhem",
    secondId: "gram",
    firstExamples: [1, 5, 10, 50, 100, 400, 1000],
    secondExamples: [5, 10, 50, 100, 500, 1000, 5000],
  },
  {
    category: "kutle",
    firstId: "okka",
    secondId: "dirhem",
    firstExamples: [0.25, 0.5, 1, 2, 5, 10, 50],
    secondExamples: [100, 200, 400, 800, 2000, 4000, 10000],
  },
  {
    category: "kutle",
    firstId: "miskal",
    secondId: "dirhem",
    firstExamples: [1, 2, 4, 10, 20, 50],
    secondExamples: [1.5, 3, 6, 15, 30, 75],
  },
  {
    category: "kutle",
    firstId: "batman",
    secondId: "okka",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [6, 12, 30, 60, 120, 300],
  },
  {
    category: "hacim",
    firstId: "kile",
    secondId: "sinik",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [4, 8, 20, 40, 80, 200],
  },
  {
    category: "hacim",
    firstId: "sinik",
    secondId: "litre",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [9.25, 18.5, 46.25, 92.5, 185, 462.5],
  },
  {
    category: "kutle",
    firstId: "litra",
    secondId: "gram",
    firstExamples: [0.25, 0.5, 1, 2, 5, 10, 50],
    secondExamples: [100, 250, 500, 1000, 2500, 5000, 10000],
  },
  {
    category: "kutle",
    firstId: "litra",
    secondId: "ounkia",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 6, 12, 24, 60, 120, 600],
  },
  {
    category: "kutle",
    firstId: "ounkia",
    secondId: "gram",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [10, 27, 50, 100, 270, 500, 1000],
  },
  {
    category: "kutle",
    firstId: "gram",
    secondId: "troy-ons",
    firstExamples: [1, 5, 10, 31.1, 50, 100, 500],
    secondExamples: [1, 2, 5, 10, 25, 50, 100],
  },
  {
    category: "kutle",
    firstId: "ons",
    secondId: "troy-ons",
    firstExamples: [1, 2, 5, 10, 16, 32, 100],
    secondExamples: [1, 2, 5, 10, 20, 50, 100],
  },
  {
    category: "kutle",
    firstId: "dirhem",
    secondId: "troy-ons",
    firstExamples: [1, 5, 10, 50, 100, 400, 1000],
    secondExamples: [0.1, 0.5, 1, 5, 10, 50, 100],
  },
  {
    category: "kutle",
    firstId: "gram",
    secondId: "karat",
    firstExamples: [0.01, 0.1, 0.2, 0.5, 1, 5, 10],
    secondExamples: [0.5, 1, 2, 5, 10, 50, 100],
  },
  {
    category: "basinc",
    firstId: "psi",
    secondId: "bar",
    firstExamples: [1, 5, 10, 15, 30, 50, 100],
    secondExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
  },
  {
    category: "basinc",
    firstId: "kilopascal",
    secondId: "bar",
    firstExamples: [1, 10, 50, 100, 250, 500, 1000],
    secondExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
  },
  {
    category: "basinc",
    firstId: "pascal",
    secondId: "bar",
    firstExamples: [1, 100, 1000, 10000, 50000, 100000],
    secondExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
  },
  {
    category: "basinc",
    firstId: "atmosfer",
    secondId: "bar",
    firstExamples: [0.5, 1, 2, 5, 10, 20, 50],
    secondExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
    englishOverride: { forward: "atmospheres-to-bar", reverse: "bar-to-atmospheres" },
  },
  {
    category: "basinc",
    firstId: "milimetre-civa",
    secondId: "pascal",
    firstExamples: [1, 5, 10, 25, 50, 100, 760],
    secondExamples: [1, 10, 100, 1000, 5000, 10000, 101325],
  },
  {
    category: "basinc",
    firstId: "kilogram-kuvvet-santimetrekare",
    secondId: "bar",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
    secondExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
    nameOverrides: {"kilogram-kuvvet-santimetrekare":"Kilogram-kuvvet/santimetrekare"},
    englishOverride: { forward: "kilogram-force-per-square-centimeter-to-bar", reverse: "bar-to-kilogram-force-per-square-centimeter" },
  },
  {
    category: "alan",
    firstId: "metrekare",
    secondId: "fitkare",
    firstExamples: [1, 5, 10, 25, 50, 100, 250],
    secondExamples: [1, 10, 50, 100, 250, 500, 1000],
    factorOverride: 10.7639104167,
  },
  {
    category: "alan",
    firstId: "yardakare",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 25, 50, 100, 250],
    secondExamples: [1, 5, 10, 25, 50, 100, 250],
  },
  {
    category: "alan",
    firstId: "hektar",
    secondId: "metrekare",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 25],
    secondExamples: [10, 100, 500, 1000, 5000, 10000, 50000],
  },
  {
    category: "hacim",
    firstId: "litre",
    secondId: "metrekup",
    firstExamples: [1, 10, 50, 100, 250, 500, 1000],
    secondExamples: [0.001, 0.01, 0.1, 0.5, 1, 2, 5],
  },
  {
    category: "hacim",
    firstId: "litre",
    secondId: "mililitre",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 25],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "zaman",
    firstId: "saniye",
    secondId: "saat",
    firstExamples: [1, 30, 60, 300, 1800, 3600, 7200],
    secondExamples: [0.25, 0.5, 1, 2, 4, 8, 12],
  },
  {
    category: "zaman",
    firstId: "dakika",
    secondId: "saat",
    firstExamples: [1, 5, 10, 15, 30, 60, 120],
    secondExamples: [0.25, 0.5, 1, 2, 4, 8, 12],
  },
  {
    category: "hiz",
    firstId: "kilometre-saat",
    secondId: "metre-saniye",
    firstExamples: [1, 5, 10, 30, 50, 90, 120],
    secondExamples: [1, 2, 5, 10, 15, 20, 30],
  },
  {
    category: "hiz",
    firstId: "mil-saat",
    secondId: "kilometre-saat",
    firstExamples: [1, 5, 10, 30, 55, 60, 75],
    secondExamples: [10, 30, 50, 80, 100, 120, 130],
  },
  {
    category: "enerji",
    firstId: "joule",
    secondId: "kilovatsaat",
    firstExamples: [1, 1000, 10000, 100000, 1000000, 3600000],
    secondExamples: [0.001, 0.01, 0.1, 0.5, 1, 2, 5],
  },
  {
    category: "aci",
    firstId: "derece",
    secondId: "radyan",
    firstExamples: [30, 45, 60, 90, 180, 270, 360],
    secondExamples: [0.5236, 0.7854, 1.0472, 1.5708, 3.1416, 4.7124, 6.2832],
  },
  {
    category: "aci",
    firstId: "derece",
    secondId: "gradyan",
    firstExamples: [9, 45, 90, 180, 270, 360, 450],
    secondExamples: [10, 50, 100, 200, 300, 400, 500],
  },
  {
    category: "frekans",
    firstId: "hertz",
    secondId: "kilohertz",
    firstExamples: [100, 500, 1000, 5000, 10000, 50000, 100000],
    secondExamples: [0.1, 0.5, 1, 5, 10, 50, 100],
  },
  {
    category: "frekans",
    firstId: "megahertz",
    secondId: "gigahertz",
    firstExamples: [100, 500, 1000, 2400, 5000, 10000, 100000],
    secondExamples: [0.1, 0.5, 1, 2.4, 5, 10, 100],
  },
  {
    category: "debi_hacimsel",
    firstId: "m3-s",
    secondId: "gpm",
    firstExamples: [0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 1],
    secondExamples: [15.85, 79.25, 158.5, 792.5, 1585, 7925, 15850],
  },
  {
    category: "debi_hacimsel",
    firstId: "m3-s",
    secondId: "cfm",
    firstExamples: [0.01, 0.05, 0.1, 0.5, 1, 5, 10],
    secondExamples: [21.19, 105.9, 211.9, 1059.4, 2118.9, 10594, 21189],
  },
  {
    category: "debi_kutlesel",
    firstId: "kg-s",
    secondId: "kg-h",
    firstExamples: [0.01, 0.1, 0.5, 1, 5, 10, 50],
    secondExamples: [36, 360, 1800, 3600, 18000, 36000, 180000],
  },
  {
    category: "manyetik_alan",
    firstId: "a-m",
    secondId: "oersted",
    firstExamples: [10, 79.5775, 100, 500, 1000, 5000, 10000],
    secondExamples: [0.1257, 1, 1.2566, 6.283, 12.566, 62.83, 125.66],
  },
  {
    category: "manyetik_aki",
    firstId: "weber",
    secondId: "miliweber",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 10, 100],
    secondExamples: [1, 10, 100, 1000, 5000, 10000, 100000],
  },
  {
    category: "viskozite_kinematik",
    firstId: "m2-s",
    secondId: "centistoke",
    firstExamples: [0.000001, 0.00001, 0.0001, 0.0005, 0.001, 0.005, 0.01],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "isil_iletkenlik",
    firstId: "w-mk",
    secondId: "btu-hftf",
    firstExamples: [0.02, 0.04, 0.5, 1, 2, 50, 400],
    secondExamples: [0.0116, 0.0231, 0.289, 0.578, 1.156, 28.9, 231.2],
  },
  {
    category: "isi_akisi",
    firstId: "w-m2",
    secondId: "kw-m2",
    firstExamples: [100, 500, 1000, 5000, 10000, 50000, 100000],
    secondExamples: [0.1, 0.5, 1, 5, 10, 50, 100],
  },
  {
    category: "ozgul_isi",
    firstId: "j-kgk",
    secondId: "cal-gk",
    firstExamples: [1, 100, 1000, 4184, 10000, 50000, 100000],
    secondExamples: [0.000239, 0.0239, 0.239, 1, 2.39, 11.95, 23.9],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "g0",
    firstExamples: [1, 5, 9.81, 15, 20, 50, 100],
    secondExamples: [0.102, 0.51, 1, 1.53, 2.04, 5.1, 10.2],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "ft-s2",
    firstExamples: [1, 5, 9.81, 15, 20, 50, 100],
    secondExamples: [3.28, 16.4, 32.19, 49.21, 65.62, 164.04, 328.08],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "rad-s",
    firstExamples: [60, 500, 1000, 1500, 3000, 6000, 10000],
    secondExamples: [6.28, 52.36, 104.72, 157.08, 314.16, 628.32, 1047.2],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "deg-s",
    firstExamples: [1, 10, 60, 100, 500, 1000, 3000],
    secondExamples: [6, 60, 360, 600, 3000, 6000, 18000],
  },
  {
    category: "guc",
    firstId: "kilowatt",
    secondId: "watt",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 50],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "debi",
    firstId: "metrekup-saat",
    secondId: "litre-dakika",
    firstExamples: [0.5, 1, 2, 5, 10, 20, 50],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "elektrik",
    firstId: "volt",
    secondId: "kilovolt",
    firstExamples: [1, 12, 24, 230, 400, 1000, 10000],
    secondExamples: [0.1, 0.5, 1, 5, 10, 20, 50],
  },
  {
    category: "elektrik",
    firstId: "amper",
    secondId: "miliamper",
    firstExamples: [0.1, 0.5, 1, 2, 5, 10, 20],
    secondExamples: [1, 5, 10, 20, 100, 500, 1000],
  },
  {
    category: "elektrik",
    firstId: "volt",
    secondId: "milivolt",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 12, 230],
    secondExamples: [1, 10, 100, 1000, 5000, 12000, 230000],
  },
  {
    category: "elektrik",
    firstId: "amper",
    secondId: "kiloamper",
    firstExamples: [100, 1000, 5000, 10000, 25000, 50000, 100000],
    secondExamples: [0.1, 1, 5, 10, 25, 50, 100],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "g-cm3",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.001, 1, 2.7, 7.85, 11.34, 19.3],
  },
  {
    category: "guc",
    firstId: "kilowatt",
    secondId: "beygirgucu-metric",
    firstExamples: [55, 66, 74, 85, 110, 140, 184],
    secondExamples: [75, 90, 105, 116, 150, 190, 245],
  },
  {
    category: "guc",
    firstId: "megawatt",
    secondId: "kilowatt",
    firstExamples: [0.5, 1, 5, 10, 50, 100, 500],
    secondExamples: [500, 1000, 5000, 10000, 50000, 100000, 500000],
  },
  {
    category: "guc",
    firstId: "beygirgucu-mechanical",
    secondId: "beygirgucu-metric",
    firstExamples: [10, 50, 100, 200, 500, 1000, 5000],
    secondExamples: [10.14, 50.7, 101.4, 202.8, 507, 1013.9, 5069.7],
  },
  {
    category: "kuvvet",
    firstId: "newton",
    secondId: "kilogram-kuvvet",
    firstExamples: [1, 10, 50, 100, 500, 1000, 10000],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "tork",
    firstId: "newton-metre",
    secondId: "lb-ft",
    firstExamples: [10, 50, 100, 200, 300, 400, 500],
    secondExamples: [10, 50, 100, 150, 200, 250, 300],
  },
  {
    category: "momentum",
    firstId: "kg-m-s",
    secondId: "n-s",
    firstExamples: [1, 10, 50, 100, 500, 1000, 10000],
    secondExamples: [1, 10, 50, 100, 500, 1000, 10000],
  },
  {
    category: "viskozite_dinamik",
    firstId: "pascal-saniye",
    secondId: "centipoise",
    firstExamples: [0.001, 0.01, 0.1, 1, 2, 5, 10],
    secondExamples: [1, 10, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "veri",
    firstId: "petabayt",
    secondId: "terabayt",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [100, 500, 1000, 2000, 4000, 8000],
  },
  {
    category: "veri",
    firstId: "terabayt",
    secondId: "gigabayt",
    firstExamples: [1, 2, 4, 8, 16, 32, 64],
    secondExamples: [100, 250, 500, 1000, 2000, 4000, 8000],
  },
  {
    category: "veri",
    firstId: "kibibayt",
    secondId: "bayt",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200, 102400],
  },
  {
    category: "veri",
    firstId: "mebibayt",
    secondId: "kibibayt",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200, 102400],
  },
  {
    category: "veri",
    firstId: "gibibayt",
    secondId: "mebibayt",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200, 102400],
  },
  {
    category: "veri",
    firstId: "tebibayt",
    secondId: "gibibayt",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200, 102400],
  },
  {
    category: "veri",
    firstId: "gigabayt",
    secondId: "megabayt",
    firstExamples: [1, 4, 8, 16, 32, 64, 128],
    secondExamples: [100, 500, 1000, 2000, 4000, 8000, 16000],
  },
  {
    category: "veri",
    firstId: "megabayt",
    secondId: "kilobayt",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [100, 500, 1000, 5000, 10000, 50000, 100000],
  },
  {
    category: "veri",
    firstId: "kilobayt",
    secondId: "bayt",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [100, 500, 1000, 5000, 10000, 50000, 100000],
  },
  {
    category: "elektrik_direnc",
    firstId: "kiloohm",
    secondId: "ohm",
    firstExamples: [0.1, 0.5, 1, 2.2, 4.7, 10, 100],
    secondExamples: [100, 220, 470, 1000, 2200, 4700, 10000],
  },
  {
    category: "kapasitans",
    firstId: "mikrofarad",
    secondId: "nanofarad",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100, 1000],
    secondExamples: [1, 10, 100, 1000, 10000, 100000, 1000000],
  },
  {
    category: "enduktans",
    firstId: "milihenry",
    secondId: "mikrohenry",
    firstExamples: [0.1, 0.5, 1, 5, 10, 50, 100],
    secondExamples: [100, 500, 1000, 5000, 10000, 50000, 100000],
  },
  {
    category: "elektrik_yuk",
    firstId: "milicoulomb",
    secondId: "mikrocoulomb",
    firstExamples: [0.1, 0.5, 1, 5, 10, 50, 100],
    secondExamples: [100, 500, 1000, 5000, 10000, 50000, 100000],
  },
  {
    category: "altin_ayar",
    firstId: "ayar-24",
    secondId: "ayar-22",
    firstExamples: [1, 5, 10, 20, 50, 100, 250],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "altin_ayar",
    firstId: "ayar-22",
    secondId: "ayar-18",
    firstExamples: [1, 5, 10, 20, 50, 100, 250],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "altin_ayar",
    firstId: "ayar-18",
    secondId: "ayar-14",
    firstExamples: [1, 5, 10, 20, 50, 100, 250],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "gumus_ayar",
    firstId: "gumus-999",
    secondId: "gumus-925",
    firstExamples: [1, 5, 10, 20, 50, 100, 250],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "gumus_ayar",
    firstId: "gumus-925",
    secondId: "gumus-900",
    firstExamples: [1, 5, 10, 20, 50, 100, 250],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "gumus_ayar",
    firstId: "gumus-900",
    secondId: "gumus-800",
    firstExamples: [1, 5, 10, 20, 50, 100, 250],
    secondExamples: [1, 5, 10, 20, 50, 100, 250],
  },
  {
    category: "kan_sekeri",
    firstId: "glukoz-mgdl",
    secondId: "glukoz-mmol",
    firstExamples: [70, 100, 126, 140, 180, 200, 250],
    secondExamples: [3.9, 5.6, 7.0, 7.8, 10, 11.1, 13.9],
  },
  {
    category: "vitamin_d",
    firstId: "vitamin-d-ngml",
    secondId: "vitamin-d-nmol",
    firstExamples: [10, 20, 30, 40, 50, 60, 100],
    secondExamples: [25, 50, 75, 100, 125, 150, 250],
  },
  {
    category: "uzunluk",
    firstId: "mikrometre",
    secondId: "milimetre",
    firstExamples: [1, 10, 50, 100, 500, 1000, 10000],
    secondExamples: [0.001, 0.01, 0.1, 1, 5, 10, 50],
  },
  {
    category: "uzunluk",
    firstId: "nanometre",
    secondId: "mikrometre",
    firstExamples: [1, 10, 100, 500, 1000, 5000, 10000],
    secondExamples: [0.001, 0.01, 0.1, 1, 5, 10, 50],
  },
  {
    category: "uzunluk",
    firstId: "deniz-mili",
    secondId: "kilometre",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "arsin",
    secondId: "metre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "arsin",
    secondId: "santimetre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [10, 50, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "uzunluk",
    firstId: "endaze",
    secondId: "metre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "arsin",
    secondId: "endaze",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 2, 5, 10, 20, 50, 100],
  },
  {
    category: "uzunluk",
    firstId: "pous",
    secondId: "metre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "pous",
    secondId: "orgyia",
    firstExamples: [1, 2, 6, 12, 30, 60, 120],
    secondExamples: [1, 2, 5, 10, 20, 50, 100],
  },
  {
    category: "uzunluk",
    firstId: "cig",
    secondId: "metre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "uzunluk",
    firstId: "cig",
    secondId: "santimetre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [10, 33, 100, 333, 1000, 3330, 10000],
  },
  {
    category: "alan",
    firstId: "santimetrekare",
    secondId: "metrekare",
    firstExamples: [1, 10, 100, 500, 1000, 5000, 10000],
    secondExamples: [0.0001, 0.001, 0.01, 0.1, 1, 10, 100],
  },
  {
    category: "alan",
    firstId: "milimetrekare",
    secondId: "santimetrekare",
    firstExamples: [1, 10, 100, 500, 1000, 5000, 10000],
    secondExamples: [0.01, 0.1, 1, 10, 50, 100, 500],
  },
  {
    category: "alan",
    firstId: "kilometrekare",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [1000000, 5000000, 10000000, 50000000, 100000000, 500000000, 1000000000],
  },
  {
    category: "alan",
    firstId: "incare",
    secondId: "santimetrekare",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [1, 10, 50, 100, 500, 1000, 5000],
  },
  {
    category: "alan",
    firstId: "akre",
    secondId: "metrekare",
    firstExamples: [1, 2, 5, 10, 50, 100, 500],
    secondExamples: [1000, 4047, 5000, 10000, 40470, 100000, 500000],
  },
  {
    category: "alan",
    firstId: "killa",
    secondId: "akre",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [1, 2, 5, 10, 20, 50],
  },
  {
    category: "alan",
    firstId: "kanal",
    secondId: "marla",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [1, 5, 10, 20, 50, 100],
  },
  {
    category: "alan",
    firstId: "guntha",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 20, 40, 100],
    secondExamples: [100, 500, 1000, 2000, 4000, 10000],
  },
  {
    category: "alan",
    firstId: "cent-arazi",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 25, 50, 100],
    secondExamples: [40, 200, 400, 1000, 2000, 4000],
  },
  {
    category: "alan",
    firstId: "ground",
    secondId: "metrekare",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [200, 450, 1100, 2200, 4500, 11000],
  },
  {
    category: "alan",
    firstId: "biswa",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 20, 50, 100],
    secondExamples: [100, 500, 1000, 2500, 6300, 12600],
  },
  {
    category: "alan",
    firstId: "donum",
    secondId: "metrekare",
    firstExamples: [0.5, 1, 2, 5, 10, 50, 100],
    secondExamples: [500, 1000, 2000, 5000, 10000, 50000, 100000],
  },
  {
    category: "alan",
    firstId: "donum",
    secondId: "dekar",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 2, 5, 10, 20, 50, 100],
  },
  {
    category: "alan",
    firstId: "donum",
    secondId: "hektar",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [0.1, 0.2, 0.5, 1, 2, 5, 10],
  },
  {
    category: "alan",
    firstId: "feddan",
    secondId: "metrekare",
    firstExamples: [0.5, 1, 2, 5, 10, 50, 100],
    secondExamples: [2100, 4201, 8402, 21004, 42008, 210042, 420083],
  },
  {
    category: "alan",
    firstId: "feddan",
    secondId: "donum",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [4.2, 8.4, 21, 42, 84, 210, 420],
  },
  {
    category: "alan",
    firstId: "feddan",
    secondId: "kirat-arazi",
    firstExamples: [0.5, 1, 2, 5, 10, 20, 24],
    secondExamples: [12, 24, 48, 120, 240, 480, 576],
  },
  {
    category: "alan",
    firstId: "kirat-arazi",
    secondId: "metrekare",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [175, 350, 875, 1750, 3501, 8752, 17503],
  },
  {
    category: "alan",
    firstId: "kirat-arazi",
    secondId: "sehm-arazi",
    firstExamples: [1, 2, 5, 10, 20, 24, 50],
    secondExamples: [24, 48, 120, 240, 480, 576, 1200],
  },
  {
    category: "alan",
    firstId: "sehm-arazi",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 24, 50, 100, 576],
    secondExamples: [7.3, 36.5, 72.9, 175, 364.7, 729.3, 4200.8],
  },
  {
    category: "alan",
    firstId: "dekar",
    secondId: "metrekare",
    firstExamples: [0.5, 1, 2, 5, 10, 50, 100],
    secondExamples: [500, 1000, 2000, 5000, 10000, 50000, 100000],
  },
  {
    category: "alan",
    firstId: "decimal-arazi",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 20, 33, 50, 100],
    secondExamples: [40.47, 202.34, 404.69, 809.37, 1335.46, 2023.43, 4046.86],
  },
  {
    category: "alan",
    firstId: "katha",
    secondId: "metrekare",
    firstExamples: [1, 2, 5, 10, 20, 33, 50],
    secondExamples: [66.89, 133.78, 334.45, 668.9, 1337.8, 2207.38, 3344.51],
  },
  {
    category: "alan",
    firstId: "bigha",
    secondId: "metrekare",
    firstExamples: [0.5, 1, 2, 3, 5, 10, 20],
    secondExamples: [668.9, 1337.8, 2675.61, 4013.41, 6689.02, 13378.04, 26756.08],
  },
  {
    category: "alan",
    firstId: "bigha",
    secondId: "katha",
    firstExamples: [0.5, 1, 2, 3, 5, 10, 20],
    secondExamples: [10, 20, 40, 60, 100, 200, 400],
  },
  {
    category: "alan",
    firstId: "katha",
    secondId: "decimal-arazi",
    firstExamples: [1, 2, 5, 10, 20, 33, 50],
    secondExamples: [1.65, 3.31, 8.26, 16.53, 33.06, 54.55, 82.64],
  },
  {
    category: "alan",
    firstId: "bigha",
    secondId: "decimal-arazi",
    firstExamples: [0.5, 1, 2, 3, 5, 10, 20],
    secondExamples: [16.53, 33.06, 66.12, 99.17, 165.29, 330.58, 661.16],
  },
  {
    category: "alan",
    firstId: "decimal-arazi",
    secondId: "akre",
    firstExamples: [10, 25, 50, 75, 100, 200, 500],
    secondExamples: [0.1, 0.25, 0.5, 0.75, 1, 2, 5],
  },
  {
    category: "hacim",
    firstId: "santimetrekup",
    secondId: "litre",
    firstExamples: [1, 50, 100, 125, 250, 500, 1000],
    secondExamples: [0.001, 0.05, 0.1, 0.5, 1, 5, 10],
  },
  {
    category: "hacim",
    firstId: "fitkup",
    secondId: "metrekup",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [1, 5, 10, 50, 100, 500, 1000],
  },
  {
    category: "hacim",
    firstId: "inckup",
    secondId: "santimetrekup",
    firstExamples: [1, 10, 50, 100, 200, 350, 500],
    secondExamples: [1, 100, 500, 1000, 2000, 5000, 10000],
  },
  {
    category: "hacim",
    firstId: "galon",
    secondId: "litre",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 5, 10, 20, 50, 100, 500],
  },
  {
    category: "enerji",
    firstId: "kalori",
    secondId: "joule",
    firstExamples: [1, 10, 50, 100, 500, 1000, 5000],
    secondExamples: [1, 10, 50, 100, 500, 1000, 5000],
  },
  {
    category: "enerji",
    firstId: "kilokalori",
    secondId: "kilojoule",
    firstExamples: [1, 50, 100, 200, 500, 1000, 2000],
    secondExamples: [1, 50, 100, 500, 1000, 4000, 8000],
  },
  {
    category: "enerji",
    firstId: "btu",
    secondId: "kilovatsaat",
    firstExamples: [1, 100, 1000, 5000, 9000, 12000, 24000],
    secondExamples: [0.1, 0.5, 1, 2, 5, 10, 50],
  },
  {
    category: "enerji",
    firstId: "watt-saat",
    secondId: "kilovatsaat",
    firstExamples: [1, 10, 100, 500, 1000, 2000, 5000],
    secondExamples: [0.001, 0.01, 0.1, 0.5, 1, 2, 5],
  },
  {
    category: "basinc",
    firstId: "milibar",
    secondId: "pascal",
    firstExamples: [1, 10, 50, 100, 500, 1000, 1013],
    secondExamples: [1, 10, 100, 1000, 10000, 50000, 101325],
  },
  {
    category: "hiz",
    firstId: "knot",
    secondId: "kilometre-saat",
    firstExamples: [1, 5, 10, 20, 30, 40, 50],
    secondExamples: [1, 5, 10, 20, 50, 100, 200],
  },
  {
    category: "zaman",
    firstId: "gun",
    secondId: "saat",
    firstExamples: [1, 2, 3, 7, 14, 30, 365],
    secondExamples: [1, 6, 12, 24, 48, 168, 720],
  },
  {
    category: "hiz",
    firstId: "isik-hizi",
    secondId: "metre-saniye",
    firstExamples: [0.001, 0.01, 0.1, 0.5, 1, 2, 10],
    secondExamples: [1000, 100000, 1000000, 10000000, 100000000, 299792458, 500000000],
  },
  {
    category: "hacim",
    firstId: "varil",
    secondId: "litre",
    firstExamples: [1, 2, 5, 10, 50, 100, 1000],
    secondExamples: [10, 50, 100, 500, 1000, 5000, 10000],
  },
  {
    category: "tork",
    firstId: "kilogram-kuvvet-metre",
    secondId: "newton-metre",
    firstExamples: [1, 5, 10, 20, 35, 50, 100],
    secondExamples: [10, 50, 100, 200, 350, 500, 1000],
  },

  // --- Asagidaki gruplar, sadece 1 birim cifti sayfasi olan (dolayisiyla
  // arama/kesif acisindan cok dar kalan) niche kategorileri unitRegistry'de
  // zaten tanimli ama hic sayfasi olmayan birimlere baglar (2026-09-02).
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "g-ml",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.001, 1, 2.7, 7.85, 11.34, 19.3],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "kg-l",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.001, 1, 2.7, 7.85, 11.34, 19.3],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "g-l",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [1, 1000, 2700, 7850, 11340, 19300],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "lb-ft3",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.06, 62.4, 168.6, 490, 708, 1205],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "lb-in3",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.0001, 0.036, 0.098, 0.284, 0.41, 0.697],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "lb-gal-us",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.01, 8.34, 22.5, 65.5, 94.6, 161],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "slug-ft3",
    firstExamples: [1, 1000, 2700, 7850, 11340, 19300],
    secondExamples: [0.002, 1.94, 5.24, 15.2, 22, 37.5],
  },
  {
    category: "yogunluk",
    firstId: "kg-m3",
    secondId: "mg-l",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100],
    secondExamples: [1, 10, 50, 100, 500, 1000],
  },
  {
    category: "viskozite_dinamik",
    firstId: "pascal-saniye",
    secondId: "milipascal-saniye",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100],
    secondExamples: [1, 10, 100, 1000, 5000, 10000],
  },
  {
    category: "viskozite_dinamik",
    firstId: "pascal-saniye",
    secondId: "poise",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100],
    secondExamples: [0.01, 0.1, 1, 10, 100, 1000],
  },
  {
    category: "viskozite_kinematik",
    firstId: "m2-s",
    secondId: "mm2-s",
    firstExamples: [0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1],
    secondExamples: [1, 10, 100, 1000, 10000, 100000],
  },
  {
    category: "tork",
    firstId: "newton-metre",
    secondId: "kilonewton-metre",
    firstExamples: [1, 10, 50, 100, 500, 1000],
    secondExamples: [0.001, 0.01, 0.05, 0.1, 0.5, 1],
  },
  {
    category: "kuvvet",
    firstId: "newton",
    secondId: "kilonewton",
    firstExamples: [1, 10, 100, 500, 1000, 5000],
    secondExamples: [0.001, 0.01, 0.1, 0.5, 1, 5],
  },
  {
    category: "kuvvet",
    firstId: "newton",
    secondId: "lbf",
    firstExamples: [1, 5, 10, 50, 100, 500],
    secondExamples: [0.22, 1.12, 2.25, 11.2, 22.5, 112],
  },
  {
    category: "kuvvet",
    firstId: "newton",
    secondId: "dyn",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100],
    secondExamples: [100, 1000, 10000, 100000, 1000000, 10000000],
  },
  {
    category: "aci",
    firstId: "derece",
    secondId: "tam-tur",
    firstExamples: [0, 90, 180, 270, 360, 720],
    secondExamples: [0.25, 0.5, 1, 2, 5, 10],
  },
  {
    category: "frekans",
    firstId: "hertz",
    secondId: "megahertz",
    firstExamples: [1, 1000, 10000, 100000, 1000000, 10000000],
    secondExamples: [0.001, 0.01, 0.1, 1, 10, 100],
  },
  {
    category: "frekans",
    firstId: "hertz",
    secondId: "gigahertz",
    firstExamples: [1000000, 10000000, 100000000, 1000000000, 2000000000, 5000000000],
    secondExamples: [0.001, 0.01, 0.1, 1, 2, 5],
  },
  {
    category: "debi_hacimsel",
    firstId: "m3-s",
    secondId: "l-s",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 10],
    secondExamples: [1, 10, 100, 1000, 5000, 10000],
  },
  {
    category: "debi_hacimsel",
    firstId: "m3-s",
    secondId: "l-min-hacimsel",
    firstExamples: [0.0001, 0.001, 0.01, 0.1, 1, 5],
    secondExamples: [1, 10, 60, 100, 1000, 5000],
  },
  {
    category: "debi_hacimsel",
    firstId: "m3-s",
    secondId: "m3-h-hacimsel",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 10],
    secondExamples: [1, 10, 100, 500, 1000, 5000],
  },
  {
    category: "debi_kutlesel",
    firstId: "kg-s",
    secondId: "g-s",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100],
    secondExamples: [1, 10, 100, 1000, 10000, 100000],
  },
  {
    category: "debi_kutlesel",
    firstId: "kg-s",
    secondId: "g-h",
    firstExamples: [0.0001, 0.001, 0.01, 0.1, 1, 10],
    secondExamples: [1, 100, 1000, 10000, 100000, 1000000],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "cm-s2",
    firstExamples: [0.01, 0.1, 1, 5, 10, 100],
    secondExamples: [1, 10, 100, 500, 1000, 10000],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "mm-s2",
    firstExamples: [0.001, 0.01, 0.1, 1, 10, 100],
    secondExamples: [1, 10, 100, 1000, 10000, 100000],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "km-s2",
    firstExamples: [100, 1000, 5000, 10000, 50000, 100000],
    secondExamples: [0.1, 1, 5, 10, 50, 100],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "in-s2",
    firstExamples: [0.1, 0.5, 1, 5, 10, 50],
    secondExamples: [4, 20, 39, 197, 394, 1969],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "ft-min2",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 10],
    secondExamples: [10, 100, 1000, 10000, 50000, 100000],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "m-min2",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 10],
    secondExamples: [1, 10, 100, 1000, 5000, 10000],
  },
  {
    category: "ivme",
    firstId: "m-s2",
    secondId: "ivme-gal",
    firstExamples: [0.01, 0.1, 1, 5, 10, 100],
    secondExamples: [1, 10, 100, 500, 1000, 10000],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "rad-min",
    firstExamples: [1, 10, 60, 100, 500, 1000],
    secondExamples: [6.28, 62.8, 377, 628, 3142, 6283],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "rad-h",
    firstExamples: [1, 10, 60, 100, 500, 1000],
    secondExamples: [377, 3770, 22619, 37699, 188496, 376991],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "deg-min",
    firstExamples: [1, 10, 60, 100, 500, 1000],
    secondExamples: [360, 3600, 21600, 36000, 180000, 360000],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "deg-h",
    firstExamples: [1, 10, 60, 100, 500, 1000],
    secondExamples: [21600, 216000, 1296000, 2160000, 10800000, 21600000],
  },
  {
    category: "acisal_hiz",
    firstId: "rpm",
    secondId: "hz",
    firstExamples: [1, 10, 60, 100, 500, 1000],
    secondExamples: [0.017, 0.167, 1, 1.667, 8.33, 16.67],
  },
  {
    category: "manyetik_alan",
    firstId: "a-m",
    secondId: "ka-m",
    firstExamples: [1, 10, 100, 1000, 5000, 10000],
    secondExamples: [0.001, 0.01, 0.1, 1, 5, 10],
  },
  {
    category: "manyetik_aki",
    firstId: "weber",
    secondId: "mikroweber",
    firstExamples: [0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1],
    secondExamples: [1, 10, 100, 1000, 10000, 100000],
  },
  {
    category: "manyetik_aki",
    firstId: "weber",
    secondId: "nanoweber",
    firstExamples: [0.000000001, 0.00000001, 0.0000001, 0.000001, 0.00001, 0.0001],
    secondExamples: [1, 10, 100, 1000, 10000, 100000],
  },
  {
    category: "kapasitans",
    firstId: "mikrofarad",
    secondId: "milifarad",
    firstExamples: [1, 10, 100, 220, 470, 1000],
    secondExamples: [0.001, 0.01, 0.1, 0.22, 0.47, 1],
  },
  {
    category: "kapasitans",
    firstId: "mikrofarad",
    secondId: "pikofarad",
    firstExamples: [0.00001, 0.0001, 0.001, 0.01, 0.1, 1],
    secondExamples: [10, 100, 1000, 10000, 100000, 1000000],
  },
  {
    category: "kapasitans",
    firstId: "mikrofarad",
    secondId: "farad",
    firstExamples: [1000, 10000, 100000, 470000, 1000000, 2200000],
    secondExamples: [0.001, 0.01, 0.1, 0.47, 1, 2.2],
  },
  {
    category: "enduktans",
    firstId: "milihenry",
    secondId: "henry",
    firstExamples: [1, 10, 100, 220, 470, 1000],
    secondExamples: [0.001, 0.01, 0.1, 0.22, 0.47, 1],
  },
  {
    category: "elektrik_yuk",
    firstId: "milicoulomb",
    secondId: "coulomb",
    firstExamples: [1, 10, 100, 500, 1000, 5000],
    secondExamples: [0.001, 0.01, 0.1, 0.5, 1, 5],
  },
  {
    category: "elektrik_yuk",
    firstId: "milicoulomb",
    secondId: "nanocoulomb",
    firstExamples: [0.00001, 0.0001, 0.001, 0.01, 0.1, 1],
    secondExamples: [10, 100, 1000, 10000, 100000, 1000000],
  },
  {
    category: "elektrik_direnc",
    firstId: "ohm",
    secondId: "megaohm",
    firstExamples: [1000, 10000, 100000, 1000000, 5000000, 10000000],
    secondExamples: [0.001, 0.01, 0.1, 1, 5, 10],
  },
  {
    category: "isil_iletkenlik",
    firstId: "w-mk",
    secondId: "kw-mk",
    firstExamples: [1, 10, 100, 500, 1000, 5000],
    secondExamples: [0.001, 0.01, 0.1, 0.5, 1, 5],
  },
  {
    category: "isil_iletkenlik",
    firstId: "w-mk",
    secondId: "w-cmk",
    firstExamples: [1, 10, 100, 500, 1000, 5000],
    secondExamples: [0.01, 0.1, 1, 5, 10, 50],
  },
  {
    category: "isi_akisi",
    firstId: "w-m2",
    secondId: "cal-cm2s",
    firstExamples: [1000, 10000, 41840, 100000, 418400, 1000000],
    secondExamples: [0.024, 0.24, 1, 2.4, 10, 24],
  },
  {
    category: "ozgul_isi",
    firstId: "j-kgk",
    secondId: "kj-kgk",
    firstExamples: [1000, 2000, 4186, 5000, 10000, 50000],
    secondExamples: [1, 2, 4.186, 5, 10, 50],
  },
  {
    category: "ozgul_isi",
    firstId: "j-kgk",
    secondId: "btu-lbf",
    firstExamples: [1000, 4186.8, 8373.6, 10000, 41868, 100000],
    secondExamples: [0.24, 1, 2, 2.39, 10, 23.9],
  },
  {
    category: "momentum",
    firstId: "kg-m-s",
    secondId: "lb-ft-s",
    firstExamples: [1, 5, 10, 50, 100, 500],
    secondExamples: [1, 5, 10, 50, 100, 500],
  },

  // --- Asagidaki gruplar, zaten guclu/populer kategorilerde (hiz, zaman,
  // basinc, enerji) unitRegistry'de tanimli ama hic sayfasi olmayan
  // birimleri baglar (2026-09-02).
  {
    category: "hiz",
    firstId: "metre-saniye",
    secondId: "km-s",
    firstExamples: [1, 10, 100, 343, 500, 1000],
    secondExamples: [0.001, 0.01, 0.1, 0.343, 0.5, 1],
  },
  {
    category: "hiz",
    firstId: "metre-saniye",
    secondId: "ft-s",
    firstExamples: [1, 5, 10, 50, 100, 343],
    secondExamples: [3.28, 16.4, 32.8, 164, 328, 1125],
  },
  {
    category: "hiz",
    firstId: "metre-saniye",
    secondId: "m-min",
    firstExamples: [1, 5, 10, 50, 100, 500],
    secondExamples: [60, 300, 600, 3000, 6000, 30000],
  },
  {
    category: "hiz",
    firstId: "metre-saniye",
    secondId: "km-min",
    firstExamples: [1, 10, 50, 100, 500, 1000],
    secondExamples: [0.06, 0.6, 3, 6, 30, 60],
  },
  {
    category: "hiz",
    firstId: "metre-saniye",
    secondId: "cm-s",
    firstExamples: [0.01, 0.1, 1, 5, 10, 50],
    secondExamples: [1, 10, 100, 500, 1000, 5000],
  },
  {
    category: "zaman",
    firstId: "saniye",
    secondId: "milisaniye",
    firstExamples: [0.001, 0.01, 0.1, 1, 5, 10],
    secondExamples: [1, 10, 100, 1000, 5000, 10000],
  },
  {
    category: "basinc",
    firstId: "pascal",
    secondId: "teknik-atmosfer",
    firstExamples: [1000, 10000, 98066.5, 500000, 1000000, 5000000],
    secondExamples: [0.0102, 0.102, 1, 5.1, 10.2, 51],
  },
  {
    category: "basinc",
    firstId: "pascal",
    secondId: "mmh2o",
    firstExamples: [9.81, 98.07, 490.33, 980.67, 4903.3, 9806.65],
    secondExamples: [1, 10, 50, 100, 500, 1000],
  },
  {
    category: "enerji",
    firstId: "joule",
    secondId: "megajoule",
    firstExamples: [1000, 10000, 100000, 1000000, 5000000, 10000000],
    secondExamples: [0.001, 0.01, 0.1, 1, 5, 10],
  },
  {
    category: "enerji",
    firstId: "joule",
    secondId: "therm",
    firstExamples: [105506, 1055060, 10550600, 105506000, 1055060000, 10550600000],
    secondExamples: [0.001, 0.01, 0.1, 1, 10, 100],
  },
  {
    category: "enerji",
    firstId: "joule",
    secondId: "quad-btu",
    firstExamples: [1.055e15, 1.055e16, 1.055e17, 1.055e18, 1.055e19, 1.055e20],
    secondExamples: [0.001, 0.01, 0.1, 1, 10, 100],
  },
];

function createTemperatureFormula(
  fromName: string,
  toName: string,
  fromUnit: string,
  toUnit: string
) {
  if (fromUnit === "C" && toUnit === "F") {
    return `${toName} = (${fromName} × 9/5) + 32`;
  }

  if (fromUnit === "F" && toUnit === "C") {
    return `${toName} = (${fromName} − 32) × 5/9`;
  }

  if (fromUnit === "C" && toUnit === "K") {
    return `${toName} = ${fromName} + 273,15`;
  }

  if (fromUnit === "K" && toUnit === "C") {
    return `${toName} = ${fromName} − 273,15`;
  }

  if (fromUnit === "C" && toUnit === "R") {
    return `${toName} = (${fromName} + 273,15) × 9/5`;
  }

  if (fromUnit === "R" && toUnit === "C") {
    return `${toName} = ${fromName} × 5/9 − 273,15`;
  }

  if (fromUnit === "F" && toUnit === "R") {
    return `${toName} = ${fromName} + 459,67`;
  }

  if (fromUnit === "R" && toUnit === "F") {
    return `${toName} = ${fromName} − 459,67`;
  }

  if (fromUnit === "C" && toUnit === "Re") {
    return `${toName} = ${fromName} × 4/5`;
  }

  if (fromUnit === "Re" && toUnit === "C") {
    return `${toName} = ${fromName} × 5/4`;
  }

  return `${toName} = ${fromName}`;
}

function createTemperatureExplanation(
  fromName: string,
  toName: string,
  fromUnit: string,
  toUnit: string
) {
  if (fromUnit === "C" && toUnit === "F") {
    return `${fromName} değeri ${toName} birimine çevrilirken önce 9/5 ile çarpılır, ardından 32 eklenir. 1 C, 33,8 F eder.`;
  }

  if (fromUnit === "F" && toUnit === "C") {
    return `${fromName} değeri ${toName} birimine çevrilirken önce 32 çıkarılır, sonra sonuç 5/9 ile çarpılır. 32 F, 0 C eder.`;
  }

  if (fromUnit === "C" && toUnit === "K") {
    return `${fromName} değeri ${toName} birimine çevrilirken 273,15 eklenir. 0 C, 273,15 K değerine karşılık gelir.`;
  }

  if (fromUnit === "K" && toUnit === "C") {
    return `${fromName} değeri ${toName} birimine çevrilirken 273,15 çıkarılır. 273,15 K, 0 C eder.`;
  }

  if (fromUnit === "C" && toUnit === "R") {
    return `${fromName} değerine 273,15 eklenip 9/5 ile çarpılır (Rankine, Fahrenheit derece aralığını kullanan mutlak bir sıcaklık ölçeğidir). 0 C, 491,67 R eder.`;
  }

  if (fromUnit === "R" && toUnit === "C") {
    return `${fromName} değeri 5/9 ile çarpılıp 273,15 çıkarılır. 491,67 R, 0 C eder.`;
  }

  if (fromUnit === "F" && toUnit === "R") {
    return `${fromName} değerine 459,67 eklenir (Rankine sıfırı mutlak sıfıra denk gelir). 0 F, 459,67 R eder.`;
  }

  if (fromUnit === "R" && toUnit === "F") {
    return `${fromName} değerinden 459,67 çıkarılır. 459,67 R, 0 F eder.`;
  }

  if (fromUnit === "C" && toUnit === "Re") {
    return `${fromName} değeri 4/5 ile çarpılır (su donma-kaynama aralığı Réaumur'de 0-80 derece olarak tanımlanır, Santigrat'ta ise 0-100 derecedir). 100 C, 80 Ré eder.`;
  }

  if (fromUnit === "Re" && toUnit === "C") {
    return `${fromName} değeri 5/4 ile çarpılır. 80 Ré, 100 C eder.`;
  }

  return `${fromName} değerini ${toName} birimine çevirin.`;
}

function createTemperaturePage(
  fromId: string,
  toId: string,
  exampleValues: number[],
  reverseSlug: string
): ConversionPage {
  const from = getRegistryUnit(fromId);
  const to = getRegistryUnit(toId);

  if (!from.tr || !to.tr) {
    throw new Error(`conversionPages: temperature unit ${fromId}/${toId} missing tr name`);
  }

  return {
    slug: `${from.tr.slug}-${to.tr.slug}`,
    category: "sicaklik",
    fromUnit: from.symbol,
    toUnit: to.symbol,
    fromName: from.tr.name,
    toName: to.tr.name,
    formula: createTemperatureFormula(
      from.tr.name,
      to.tr.name,
      from.symbol,
      to.symbol
    ),
    explanation: createTemperatureExplanation(
      from.tr.name,
      to.tr.name,
      from.symbol,
      to.symbol
    ),
    exampleValues,
    reverseSlug,
    englishSlug: from.en && to.en ? `${from.en.slug}-to-${to.en.slug}` : undefined,
    englishReverseSlug: from.en && to.en ? `${to.en.slug}-to-${from.en.slug}` : undefined,
  };
}

const customConversionPages: ConversionPage[] = [
  createTemperaturePage(
    "santigrat",
    "fahrenhayt",
    [-40, 0, 20, 37, 100],
    "fahrenhayt-santigrat"
  ),
  createTemperaturePage(
    "fahrenhayt",
    "santigrat",
    [-40, 32, 68, 98.6, 212],
    "santigrat-fahrenhayt"
  ),
  createTemperaturePage(
    "santigrat",
    "kelvin",
    [-40, 0, 20, 100, 200],
    "kelvin-santigrat"
  ),
  createTemperaturePage(
    "kelvin",
    "santigrat",
    [233.15, 273.15, 293.15, 310.15, 373.15],
    "santigrat-kelvin"
  ),
  createTemperaturePage(
    "santigrat",
    "rankine",
    [-273.15, -40, 0, 20, 100],
    "rankine-santigrat"
  ),
  createTemperaturePage(
    "rankine",
    "santigrat",
    [0, 491.67, 528.27, 559.67, 671.67],
    "santigrat-rankine"
  ),
  createTemperaturePage(
    "fahrenhayt",
    "rankine",
    [-459.67, -40, 0, 32, 212],
    "rankine-fahrenhayt"
  ),
  createTemperaturePage(
    "rankine",
    "fahrenhayt",
    [0, 459.67, 491.67, 528.27, 671.67],
    "fahrenhayt-rankine"
  ),
  createTemperaturePage(
    "santigrat",
    "reaumur",
    [-40, 0, 20, 37, 100],
    "reaumur-santigrat"
  ),
  createTemperaturePage(
    "reaumur",
    "santigrat",
    [-32, 0, 16, 29.6, 80],
    "santigrat-reaumur"
  ),
];

const newUnitPairDefinitions: ConversionPairDefinition[] = [
  {
    category: "uzunluk",
    firstId: "astronomik-birim",
    secondId: "kilometre",
    firstExamples: [0.5, 1, 2, 5, 10, 30, 40],
    secondExamples: [1000000, 50000000, 149597870, 500000000, 1000000000],
  },
  {
    category: "uzunluk",
    firstId: "isik-yili",
    secondId: "kilometre",
    firstExamples: [0.001, 0.01, 0.1, 1, 4.24, 10, 100],
    secondExamples: [1000000000, 9460730472580.8, 50000000000000],
  },
  {
    category: "uzunluk",
    firstId: "parsek",
    secondId: "isik-yili",
    firstExamples: [0.5, 1, 5, 10, 100, 1000],
    secondExamples: [1, 3.26, 10, 32.6, 100, 326],
  },
  {
    category: "uzunluk",
    firstId: "angstrom",
    secondId: "nanometre",
    firstExamples: [1, 5, 10, 50, 100, 500, 1000],
    secondExamples: [0.1, 1, 5, 10, 50, 100],
  },
  {
    category: "uzunluk",
    firstId: "fathom",
    secondId: "metre",
    firstExamples: [1, 5, 10, 25, 50, 100, 500],
    secondExamples: [1, 2, 10, 50, 100, 500, 1000],
  },
  {
    category: "kutle",
    firstId: "dalton",
    secondId: "gram",
    firstExamples: [1e23, 6.022e23, 1e24, 1e25],
    secondExamples: [1, 12, 18, 44, 100],
  },
  {
    category: "enerji",
    firstId: "elektronvolt",
    secondId: "joule",
    firstExamples: [1, 1000, 1000000, 1000000000],
    secondExamples: [1, 10, 100, 1000],
  },
  {
    category: "basinc",
    firstId: "torr",
    secondId: "milimetre-civa",
    firstExamples: [1, 10, 100, 500, 760, 1000],
    secondExamples: [1, 10, 100, 500, 760, 1000],
  },
  {
    category: "hacim",
    firstId: "ingiliz-galonu",
    secondId: "galon",
    firstExamples: [1, 2, 5, 10, 20, 50, 100],
    secondExamples: [1, 2, 5, 10, 20, 50, 100],
  },
  {
    category: "hacim",
    firstId: "bushel",
    secondId: "galon",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [1, 5, 10, 20, 50, 100],
  },
  {
    category: "hacim",
    firstId: "peck",
    secondId: "bushel",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [1, 2, 4, 8, 16, 32],
  },
  {
    category: "hacim",
    firstId: "sivi-ons",
    secondId: "pint",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [1, 2, 5, 10, 20],
  },
  {
    category: "hacim",
    firstId: "ingiliz-sivi-ons",
    secondId: "sivi-ons",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [1, 2, 5, 10, 20, 50],
  },
  {
    category: "hacim",
    firstId: "pint",
    secondId: "galon",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [1, 2, 5, 10, 20],
  },
  {
    category: "hacim",
    firstId: "ingiliz-pint",
    secondId: "pint",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [1, 2, 5, 10, 20, 50],
  },
  {
    category: "hacim",
    firstId: "quart",
    secondId: "pint",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [1, 2, 4, 8, 16, 32],
  },
  {
    category: "hacim",
    firstId: "ingiliz-quart",
    secondId: "quart",
    firstExamples: [1, 2, 5, 10, 20, 50],
    secondExamples: [1, 2, 5, 10, 20, 50],
  },
  {
    category: "veri",
    firstId: "kibibit",
    secondId: "bit",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [128, 1024, 2048, 5120, 10240],
  },
  {
    category: "veri",
    firstId: "mebibit",
    secondId: "kibibit",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200],
  },
  {
    category: "veri",
    firstId: "gibibit",
    secondId: "mebibit",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200],
  },
  {
    category: "veri",
    firstId: "tebibit",
    secondId: "gibibit",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1024, 2048, 5120, 10240, 51200],
  },
  {
    category: "veri",
    firstId: "megabit",
    secondId: "megabayt",
    firstExamples: [1, 8, 10, 50, 100, 500, 1000],
    secondExamples: [1, 5, 10, 25, 50, 100, 500],
  },
  {
    category: "veri",
    firstId: "kilobit",
    secondId: "megabit",
    firstExamples: [1, 10, 100, 500, 1000, 5000],
    secondExamples: [1, 2, 5, 10, 50, 100],
  },
  {
    category: "veri",
    firstId: "gigabit",
    secondId: "megabit",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1000, 2000, 5000, 10000, 50000],
  },
  {
    category: "veri",
    firstId: "terabit",
    secondId: "gigabit",
    firstExamples: [1, 2, 5, 10, 50, 100],
    secondExamples: [1000, 2000, 5000, 10000, 50000],
  },
  {
    category: "hacim",
    firstId: "yemek-kasigi",
    secondId: "mililitre",
    firstExamples: [1, 2, 4, 8, 16, 32],
    secondExamples: [15, 30, 60, 120, 250, 500],
  },
  {
    category: "hacim",
    firstId: "su-bardagi",
    secondId: "yemek-kasigi",
    firstExamples: [1, 2, 3, 4, 5, 10],
    secondExamples: [1, 5, 10, 15, 20, 25],
  },
  {
    category: "hacim",
    firstId: "cay-kasigi",
    secondId: "yemek-kasigi",
    firstExamples: [1, 3, 6, 9, 12, 15],
    secondExamples: [1, 2, 3, 4, 5, 10],
  },
  {
    category: "kutle",
    firstId: "stone",
    secondId: "kilogram",
    firstExamples: [1, 2, 5, 10, 15, 20, 25],
    secondExamples: [1, 5, 10, 50, 70, 100, 150],
  },
  {
    category: "alan",
    firstId: "tsubo",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 25, 50, 100],
    secondExamples: [1, 5, 10, 25, 50, 100],
  },
  {
    category: "basinc",
    firstId: "megapascal",
    secondId: "kilopascal",
    firstExamples: [1, 5, 10, 25, 50, 100],
    secondExamples: [100, 500, 1000, 5000, 10000, 50000],
  },
  {
    category: "basinc",
    firstId: "hektopascal",
    secondId: "pascal",
    firstExamples: [1, 10, 100, 500, 1000, 1013.25],
    secondExamples: [100, 1000, 10000, 50000, 100000, 101325],
  },
  {
    category: "guc",
    firstId: "sogutma-tonu",
    secondId: "kilowatt",
    firstExamples: [0.5, 1, 1.5, 2, 3, 5, 10],
    secondExamples: [1, 3.5, 5, 7, 10, 17.6, 35.2],
  },
  {
    category: "guc",
    firstId: "btu-saat",
    secondId: "watt",
    firstExamples: [5000, 9000, 12000, 18000, 24000, 36000],
    secondExamples: [1465, 2638, 3517, 5276, 7034, 10551],
  },
  {
    category: "kutle",
    firstId: "kental",
    secondId: "kilogram",
    firstExamples: [1, 5, 10, 25, 50, 100],
    secondExamples: [100, 500, 1000, 2500, 5000, 10000],
  },
  {
    category: "alan",
    firstId: "ar",
    secondId: "metrekare",
    firstExamples: [1, 5, 10, 25, 50, 100, 500],
    secondExamples: [100, 500, 1000, 2500, 5000, 10000],
  },
  {
    category: "alan",
    firstId: "ar",
    secondId: "hektar",
    firstExamples: [1, 5, 10, 25, 50, 100, 500],
    secondExamples: [0.01, 0.05, 0.1, 0.5, 1, 5, 10],
  },
];

export const conversionPages: ConversionPage[] = [
  ...pairDefinitions.flatMap(createConversionPair),
  ...newUnitPairDefinitions.flatMap(createConversionPair),
  ...customConversionPages,
];
